/**
 * Matcha Studio — Masterclass 3-Panel Pro Visual Studio Suite.
 *
 * Professional Dark UI Architecture inspired by Figma & Linear.
 * Vector SVG icon design system, Interactive Tag Manager & Progressive Pagination.
 *
 * @package Matcha_Gallery\Studio
 */

import { getState, setState, subscribe, patchConfig, setViewport } from './store/useStudioStore.js';
...
if (root) {
  ...
  document.getElementById('studio-shortcode').addEventListener('click', e => {
    ...
    setTimeout(() => { e.target.textContent = text; }, 1400);
  });
  ...
  // Scorecard widget triggers photo inspector
    ...
    return `
        ...
        <div class="matcha-tray-toolbar" style="display:flex;align-items:center;gap:6px;margin-bottom:12px;">
          <div style="position:relative;flex:1;display:flex;align-items:center;">
            ...
            ${traySearchQuery ? `<button type="button" id="btn-clear-tray-search" style="position:absolute;right:6px;background:none;border:none;color:#94a3b8;cursor:pointer;font-size:12px;padding:0;line-height:1;" title="Clear Search">×</button>` : ''}
          </div>
  ...
  // --- Right Sidebar: Live Properties & Photo Inspector ---
  ...
  function wallPropertiesHTML(cfg) {
  ...
  function photoInspectorHTML(id, cfg) {
    ...
    // Compute existing gallery tags available for quick assign
    const allGalleryTags = new Set();
          ...
          <div id="inspector-tag-cloud" style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:8px;">
            ${keywords.map(k => `
              <span class="matcha-badge matcha-badge--ai" style="display:inline-flex;align-items:center;gap:5px;padding:3px 8px;font-size:10px;">
                ...
                <button type="button" class="tag-del-btn" data-del-tag="${escapeHtml(k)}" style="background:none;border:none;color:#94a3b8;cursor:pointer;padding:0;font-size:12px;line-height:1;" title="Remove tag">×</button>
              </span>
            `).join('')}
          </div>
  ...
  function bindWallProperties() {
    ...
    // Accent Color Presets & Custom Input
  ...
  function bindPhotoInspector(id) {
    ...
    if (cropperBox) {
      {
        ...
        const handlePointer = (e) => {
          ...
          const curZoom = isPro ? parseFloat(zoomInp?.value || 1.0) : 1.0;
    ...
    xInp?.addEventListener('input', e => {
      ...
      const curY = parseInt(yInp?.value || 50);
      const curZ = isPro ? parseFloat(zoomInp?.value || 1.0) : 1.0;
    ...
    document.getElementById('btn-reset-ai-focal')?.addEventListener('click', () => {
      ...
      updateCrop(aiFp.x, aiFp.y, isPro ? (aiFp.zoom || 1.0) : 1.0);
    });

    // Interactive Tag Management (Single & Multi-Select)
    function addTags(rawInput) {
      if (!rawInput) return;
      const tagsToAdd = (Array.isArray(rawInput) ? rawInput : rawInput.split(','))
    ...
    function renderTagSuggestions(filterText = '') {
      ...
      // Get all unique tags across the whole gallery with counts
      ...
      // Comma-separated or new tag preview: "➕ Add 3 tags: [a], [b], [c]"
      if (query !== '') {
      ...
      // Existing gallery tags list with Multi-Select Checkboxes
      ...
      // Bind click on suggestions (toggle & keep open)
  ...
  function bindImages() {
    document.getElementById('matcha-add-images')?.addEventListener('click', () => {
      ...
      frame.on('select', () => {
        ...
        const merged = [...new Set([...cur, ...ids])]; // Unlimited 1,000+ photo gallery support
        ...
        if (currentSection) {
          currentSection.imageIds = [...new Set([...(currentSection.imageIds || []), ...ids])];
    ...
    // Segmented View Mode Toggle (List / 2-Col / 3-Col)
    ...
    sortDropdown?.querySelectorAll('.tray-menu-item').forEach(item => {
      item.addEventListener('click', (e) => {
        ...
        // Pro features gate: Newest, Oldest, Random Shuffle
    ...
    // Auto-load next batch on scroll in the left panel
    ...
    document.querySelectorAll('.matcha-blueprint-card').forEach(card => {
      card.addEventListener('click', () => {
        ...
        if (!isPro && (layout === 'pinwheel' || layout === 'bento')) {
          ...
          return; // Strictly preserve canvas and existing selection!
        }
  ...
  async function runAI() {
    ...
    // Only process images that actually need AI enrichment
    ...
    // Concurrency 1 with gentle pacing to respect Gemini Free Tier limits (15 RPM)
    while (queue.length) {
        ...
        try {
          ...
          if (json.metadata) {
            metaCache.set(attId, {
              ...
              focal_point: json.metadata.focal_point || { x: 50, y: 50, zoom: 1.0 }
            });
  ...
  function updateSingleCardMeta(attId) {
  ...
  // Global helper for infinite scroll in tray
  window.__matchaTrayLoadMore = () => {
  ...
  // --- Image List Rendering with Progressive Scalability (Supports 1,000+ photos) ---
  async function renderImageList() {
    ...
    // Progressive Chunking for 1,000+ photo performance
    ...
    list.innerHTML = visibleIds.map(id => {
      ...
      return `
        <div class="matcha-img-card ${isSelected ? 'is-selected' : ''}" data-id="${id}" title="${escapeHtml(cached?.title || media?.title?.rendered || '#' + id)}">
          ...
          <button type="button" class="remove" data-remove="${id}" title="${currentSection ? 'Remove from this chapter' : 'Remove photo from gallery'}">×</button>
        </div>
      `;
    }).join('');
    ...
    // Chunked REST API Fetching for un-cached images (Batches of 50 to avoid WP per_page=100 ceiling)
    ...
    list.querySelectorAll('[data-remove]').forEach(b => {
      b.addEventListener('click', e => {
        ...
        if (currentSection) {
          // Remove from this specific chapter
          currentSection.imageIds = (currentSection.imageIds || []).filter(x => x !== id);
          patchConfig({ sections });
        } else {
          // Remove from entire gallery and clean up references across all chapters
          const newSections = sections.map(sec => ({
            ...sec,
            imageIds: (sec.imageIds || []).filter(x => x !== id)
          }));
          patchConfig({
            imageIds: (cfg.imageIds || []).filter(x => x !== id),
    ...
    // Initialize silky-smooth SortableJS on the rendered cards
    initSortable();
  }
  ...
  function initSortable() {
    ...
    // Allow drag-and-drop reordering when not actively filtering by tag or searching
    ...
    studioSortableInstance = new Sortable(list, {
      ...
      onEnd: (evt) => {
        ...
        setTimeout(() => { didJustDrag = false; }, 80);
  ...
  function openFilterManagerModal() {
    ...
    modal.querySelector('#btn-select-all-tags')?.addEventListener('click', () => {
      modal.querySelectorAll('.tag-vis-check').forEach(cb => { cb.checked = true; });
    });
    modal.querySelector('#btn-deselect-all-tags')?.addEventListener('click', () => {
      modal.querySelectorAll('.tag-vis-check').forEach(cb => { cb.checked = false; });
    });
  ...
  // --- Live Interactive WYSIWYG Canvas ---
  async function renderCanvas() {
    ...
    function applyCanvasFilter() {
      ...
      // Pagination Logic for Canvas
      const pagType = cfg.paginationType || 'none';
      const perPage = cfg.itemsPerPage || 12;

      if (pagType === 'none') {
        matchingItems.forEach(item => { item.style.display = ''; item.style.opacity = '1'; });
  ...
  // --- Subscriptions & Autosave ---
  let lastSubscribedImageIds = null;
  subscribe(s => {
    const curIds = (s.config?.imageIds || []).join(',');
    // Only re-render image list if the actual set/count of photos in the gallery changed
    // and NOT while or right after dragging to reorder
  ...
  async function autosave(manual = false) {
    ...
    try {
      // 1. Save modified attachment metadata (keywords, alt, title) to WordPress DB
      ...
      // 2. Save Gallery CPT config
      const url = st.galleryId
        ? `${window.MatchaStudio.root}matcha-gallery/v1/galleries/${st.galleryId}`
        : `${window.MatchaStudio.root}matcha-gallery/v1/galleries`;
